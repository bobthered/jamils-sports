<script lang="ts">
	...
	// $derived
	const featuredGames = $derived.by(() => data.games.slice(0, 3));
	const latestArticles = $derived.by(() => data.articles.slice(0, 3));
</script>
...
<Section class="bg-gray-100 text-center dark:bg-gray-900">
	<Container>
		<Div class="flex flex-col space-y-12">
			...
			>
				{#each data.teams as { href, imageSrc, losses, name, wins }}
					<Card
						class="flex scale-100 flex-col overflow-hidden p-0 transition duration-200 hover:scale-105 hover:shadow-xl"
					>
		...
		<Div class="flex flex-col space-y-12">
			...
			<Card class="relative overflow-auto p-0">
				<Table class="w-full">
					...
					<Tbody>
						{#each data.teams as { losses, name, wins }, index}
							<Tr>
